...
public class GeneratorScript : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        screenWidthInPoints = height * Camera.main.aspect;      // aspect = 세로 크기를 1로 했을떄의 가로 크기의 비율
    ...
    private void GeneralRoomIfReauired()
    {
        ...
        foreach (var room in currentRooms)
        {
            ...
            // 방이 한개라도 왼쪽 끝 위치가 방추가 기준 위치보다 오른쪽에 있으면 방 추가x
            if (roomStartX > addRoomX)
                addRooms = false;

            // 방 삭제 기준 위치보다 왼쪽에 존재하는 방이 있으면 방삭제 목록에 추가
            if (roomEndX < removeRoomX)
                roomsToRemove.Add(room);

            // 가장 오른쪽 방의 오른쪽 끝 위치를 최댓값 메소드를 이용하여 구함
